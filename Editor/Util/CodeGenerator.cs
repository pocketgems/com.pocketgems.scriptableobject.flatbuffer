    ...
    internal static class CodeGenerator
    {
        ...
        public static bool GenerateFlatBufferStructs(string schemaFilePath, string outputDirectory, out List<string> errors)
        {
            ...
            var executeDirectory =
                AttemptToCreateDirPath(Path.Combine("Library", "PackageCache"), $"{ParameterConstants.PackageName}@*") ??
                AttemptToCreateDirPath("Packages", ParameterConstants.PackageName);

#if UNITY_EDITOR_OSX
            var executePath = Path.Combine(executeDirectory, "flatc");
#else
            var executePath = Path.Combine(executeDirectory, "flatc.exe");
#endif

            Process process = new Process();
            process.StartInfo.FileName = executePath;
            process.StartInfo.WorkingDirectory = outputDirectory;
            process.StartInfo.Arguments = $"--csharp \"{schemaFilePath}\""; // quotes around path in case of spaces
